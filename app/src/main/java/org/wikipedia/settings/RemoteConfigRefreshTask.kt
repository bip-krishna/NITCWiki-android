...
class RemoteConfigRefreshTask() : RecurringTask() {
    ...
    override suspend fun run(lastRun: Date) {
        val config = ServiceFactory.getRest(NITCWikiApp.instance.wikiSite).getConfiguration()
        RemoteConfig.updateConfig(config)

        if (AccountUtil.isLoggedIn) {
            val userInfo = ServiceFactory.get(NITCWikiApp.instance.wikiSite).getUserInfo()

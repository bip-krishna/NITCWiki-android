...
class MainActivity : SingleFragmentActivity<MainFragment>(), MainFragment.Callback {
    ...
    override fun onTabChanged(tab: NavTab) {
        if (tab == NavTab.HOME) {
            ...
            applyNavBarTheme(if ((fragment.currentFragment as? HomeFragment)?.getCurrentTab() == HomeTab.FOR_YOU) Theme.BLACK else NITCWikiApp.instance.currentTheme)
        } else {
            ...
            applyNavBarTheme(NITCWikiApp.instance.currentTheme)
        }
        applyInsets()
    }
        ...
        if (Intent.ACTION_VIEW == intent.action && intent.data != null) {
            ...
            intent.data?.let {
                if (it.authority.orEmpty().endsWith(WikiSite.BASE_DOMAIN)) {
                    ...
                    val oldScheme = if (BuildConfig.IS_NITC_WIKI) "nitcwiki://" else "wikipedia://"
                    val uri = it.toString().replace(oldScheme, WikiSite.DEFAULT_SCHEME + "://").toUri()
